...
IMPLEMENT_GLOBAL_SHADER(FSimplePassThroughVS, "/SpoutShaders/Private/PixelShader.usf", "MainVertexShader", SF_Vertex);
IMPLEMENT_GLOBAL_SHADER(FPixelShaderExamplePS, "/SpoutShaders/Private/PixelShader.usf", "MainPixelShader", SF_Pixel);
...
void FPixelShaderExample::DrawToRenderTarget_RenderThread(FRHICommandListImmediate& RHICmdList, const FShaderUsageExampleParameters& DrawParameters)//, FTextureRHIRef ComputeShaderOutput)
{
	...
	PassParameters.InputTexture = DrawParameters.InputTexture;
	PassParameters.InputTextureSampler = TStaticSamplerState<SF_Bilinear, AM_Wrap, AM_Wrap, AM_Wrap>::GetRHI();
	PassParameters.FrameNumber = DrawParameters.FrameNumber;
	PassParameters.ProjectionMatrix = DrawParameters.ProjectionMatrix;
	PassParameters.ViewMatrix = DrawParameters.ViewMatrix;
